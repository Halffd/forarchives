...
class MoeSearcher:
    ...
    async def multiArchiveSearch(self, archives=[0, 1, 2, 3], **query):
        ...
        self.log_dataframe(results, query=query.pop('text', ''),**query, folderName='dumpster')
    ...
    async def fetch_thread(self, session, archive_url, board, thread_num, semaphore, delay, **kwargs):
        async with semaphore:
            await asyncio.sleep(delay)
            t await thread(archive_url, board=board, thread_num=thread_num, **kwargs)
    ...
    async def searchInSubject(self, archive=0, subject='', searchText='', **kwargs):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            subjs = []
            for i in threads:
                print(threads[i].columns)
                print(threads[i].head())
                for thr in threads[i].values:
...
if __name__ == '__main__':
    ...
    results = searcher.qSearch(0, text='mushoku umaru')
    #results = searcher.qSearch(0, text='aparecida', limit=35)
    #print(results)
    #print(searcher.getText(results))
    
    results = asyncio.run(searcher.searchInSubject(0, subject='umaru', searchText='lisp|sicp'))
    print(results)
