...
class MoeSearcher:
    ...
    def posts_to_dataframe(self, posts, **kwargs):
    ...
    async def searchInSubject(self, archive=0, subject='', searchText='', **kwargs):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for index, row in subjects.iterrows():
                si += 1
                threadN = self.utilities.df_get(subjects, index, 'thread_num')
                print(f'{si}/{len(subjects)}: {threadN}')
                if threadN is None:
                    continue
                if bboard is not None board = self.utilities.df_get(subjects, index, 'board')
                short_name = board_value.get('short_name') if isinstance(board_value, dict) else None
    ...
    async def fetch_thread(self, session, archive_url, board, thread_num, semaphore, delay, **kwargs):
        async with semaphore:
            ...
            df = self.posts_to_dataframe(thr)
            return df
    def getArchive(self, archive=0):
        if isinstance(archive, int):
            index = archive
