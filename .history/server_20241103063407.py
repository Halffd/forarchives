...
class SearchHandler(tornado.web.RequestHandler):
    async def post(self):
        ...
        archive = int(self.get_argument("archive", 0))

        # Initialize MoeSearcher and perform asynchronous search
        moe_searcher = ms.MoeSearcher()
        try:
            # Fetch results from selected archive
            search_results = await moe_searcher.search(archive=archive, text=query)
