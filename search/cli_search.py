...
async def main():
    ...
    parser.add_argument(
        "--delay", type=float, default=3.0, help="Delay between requests in seconds"
    )
    parser.add_argument(
    ...
    if args.save:
        output_path = args.output or os.path.join(
            utils.log_dir, f"search_{utils.clean_filename(args.query)}.{args.format}"
        )
