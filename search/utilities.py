...
class Utilities:
    ...
    def get_log_directory(self):
        ...
        if system == "windows":
            # Windows: %APPDATA%\ForArchives\logs
            appdata = os.getenv("APPDATA")
            return os.path.join(appdata or home, "ForArchives", "logs")
        elif system == "darwin":
            # macOS: ~/Library/Logs/ForArchives
            return os.path.join(home, "Library", "Logs", "ForArchives")
        else:
            # Linux/Unix: ~/.local/share/forarchives/logs
            return os.path.join(home, ".local", "share", "forarchives", "logs")
    ...
    def get_log_file(self, site, board="", query="", folderName="", fileType=""):
        ...
        path = (
            [self.log_dir, f"{year}-{month}", day]
            if not jsonF
            else [self.log_dir, "json"]
        )
        ...
        log_file_name = (
            f"{formatted_string}-{params_str}.{fileType}"
            if params_str
            else f"{formatted_string}.{fileType}"
        )
    ...
    def df_get(self, df, row_index, column_name):
        ...
        try:
            a = df.values
            b = df.values[row_index]
            print(a, b)
            c = df.values[row_index][column_name]
            bb = df.values[column_name]
            print(bb, c)
            return df.values[row_index][column_name]
            ...
            for term in search_terms.split():
                if term.startswith("-"):
                    ...
                    found_items = [
                        ...
                        if not re.search(negated_term, item)
                    ]
    ...
    def serialize_object(self, obj):
        ...
        if isinstance(obj, dict):
            ...
            return self.serialize_object(
                vars(obj)
            )  # Use vars() for better compatibility
        else:
